...
const Relation = () => {
  ...
  const handleCalculateRelationship = () => {
    ...
    const commonLetters = new Set([...name1Set].filter(letter => name2Set.has(letter)));
    
    const remainingName1 = name1.replace(new RegExp(`[${Array.from(commonLetters).join('')}]`, 'g'), '');
    const remainingName2 = name2.replace(new RegExp(`[${Array.from(commonLetters).join('')}]`, 'g'), '');
    
    const relationshipValue = (remainingName1.length + remainingName2.length) % 6;
